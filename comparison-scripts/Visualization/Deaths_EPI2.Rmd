...
png("age_supp2b.png",width = 850, height=500)
...
write.csv(averted_age, "Deaths_EPI2_HP13.csv")
```
...
table$`% Reduction in Burden (HP13)` <- ifelse(table$Team %in% table1$Team, 100 * (table1$`Cum. Deaths per 100K` - table$`Cum. Deaths per 100K`) / table1$`Cum. Deaths per 100K`, NA)
...
averted_age <- rbind(averted_age,
                     data.frame(scenario = scenario,
                          ...
                          reduction = 100*(ages_1$Cml[which(ages_1$Bin == "sm.mean")] - 
                                       ages$Cml[which(ages$Bin == "sm.mean")]) / 
                            ages_1$Cml[which(ages_1$Bin == "sm.mean")]))
write.csv(averted, "Deaths_EPI2.csv")
write.csv(averted_age, "Deaths_age_EPI2.csv")
```